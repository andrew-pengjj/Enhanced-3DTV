function [A_hat E_hat iter] = exact_alm_WNNMrpca(D, C, tol, maxIter)
%加权核范数的用来精确RPCA的方法，没有迭代的reweighted情况
%用精确ALM方法求解下面的问题
%      min  |A|_w,*+|E|_1;    s.t. A+E=D
%     其中 w_i = C*sqrt(m*n)/(sigmaX_i+eps)
% while ~converged 
%   minimize
...
while ~converged       
    ...
    while primal_converged == false
        ...
        %[tempDiagS,svp]=ClosedWNNM(diagS,C/mu,eps);
%         sv
%         svp
        temp_A=U(:,1:svp)*diag(tempDiagS)*V(:,1:svp)';          %这里更新A
        %%%%%%%%%%%%%%%%%%

...
for iter = 1 : maxIter
    ...
    % FFT 
%     diffT_p  = diff_xT(beta(1)*U_x*V_x'-M1,sizeD)+diff_yT(beta(2)*U_y*V_y'-M2,sizeD);
%     diffT_p  = diffT_p + diff_zT(beta(3)*U_z*V_z'-M3,sizeD);
%     numer1   = reshape( diffT_p + beta(5)*(x(:)-e(:)) + M5(:), sizeD);

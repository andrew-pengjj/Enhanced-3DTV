...
% tensor product sparsity.
%
% Usage: b = A_cscamtensor(w, OMEGA, idx, P, h)
%
% w -   SxN^2 vector of wavelet coefficients
%
...
function [b,bm,tmp,x] = A_cscamtensor(w, OMEGA, idx, P, h)
...
for i=1:S,
    ...
    fx = fastwalsh(tmp(P,i),idx)/sqrt(N);
    bm(:,i) = fx(OMEGA);
end
b = bm(:);
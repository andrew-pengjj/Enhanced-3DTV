function [x,psnrpath] = LrApprReTV(A, y, sizeD, rk, lam, opts)
...
for iter = 1 : maxIter
    ...
    % x subproblem
    x= myPCG(A,y,L,x,f,v1,v2,v3,e,beta,sizeD);
...
%     x  = x(:);
    ...
    for i=1:3
        ...
        f      = softthre( diff_x + v1/beta(1), lam.*w/beta(1) );
        w=p./((abs(f)+0.001).^(1-p));
    end
    %- updating multipliers
	v1 = v1 - gamma*beta(1)*(f - diff_x);
